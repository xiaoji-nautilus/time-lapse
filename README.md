...
我手上有一个米家1080P智能**摄像机**（一般用于监控用途，130度广角，支持网络存储监控录像），一个小米路由器（带1T硬盘存储，支持**NAS**[^注1]），一个**放大镜**（**可选**），一台Windows**电脑**。还有两盆即将开花的**橡皮花**。
...
为了做到延时摄影[^注2]效果，我们需要把摄像机记录下来的连续的视频，按照一定的时间间隔（我设置了时间间隔为24秒），取出相应时间点的图像，以图片的形式存储到本地磁盘上；
...
1、后面会考虑把视频图像抓取和合并视频的功能，放到淑梅派中，这样就不需要将电脑一直开着来处理视频了
